...
fun SelectCropScreen(
    ...
    val currentSelectedCropResId = remember(appData.selectedCrop) {
        appData.selectedCrop?.let { cropString ->
            Log.i("SelectCropScreen", "Current selected crop: $cropString")
            when(cropString) {
                context.getString(R.string.crop_corn) -> R.string.crop_corn
                context.getString(R.string.crop_tomato) -> R.string.crop_tomato
                context.getString(R.string.crop_pepper) -> R.string.crop_pepper
                else -> R.string.crop_unknown
            }
        }
    }
            ...
            LazyVerticalGrid(
                ...
                items(cropOptions) { crop ->
                    ...
                    CropTileButton(
                        ...
                        onClick = {
                            Log.i("SelectCropScreen::ONClick", "Selected crop: ${context.getString(crop.nameResId)}")
                            appViewModel.setSelectedCrop(
                                context.getString(crop.nameResId)
                            )
                        }
                    )
                }
